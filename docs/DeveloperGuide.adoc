...
income or expense.
. The `transaction` selected is extracted and modified based on the command entered by the user.
. A new `transaction` modified from the `transaction` selected is created.
...
NOTE: The lifeline for `WalletEditCommand` should end at the destroy marker (X) but due to a limitation of PlantUML,
the lifeline reaches the end of diagram.
