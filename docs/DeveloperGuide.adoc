...
* The following is an in-depth view of how `ModelManager` is implemented:
...
====
The _Model_ package consists of four main packages: _Person_, <<transaction, _Transaction_>>,
...
image::ModelPersonDiagram.png[width=1200]
...
* The _Reminder_ package only consist of a class, `Reminder`:

.Reminder class diagram
image::ModelReminderClassDiagram.png[width=200]
...
.. Prerequisites: Connected to the Internet. The person who you want to remind exists in the person list. The person has at least one loan in the loan list.
...
.. Prerequisites: Connected to the Internet. At least one person in the person list has at least a loan.
