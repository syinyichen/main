...
The `owe` command is implemented in the class `PeopleOweCommand`.
...
The following activity diagram illustrates what happens when the user enters an `owe` command:

.Activity diagram of the recording a debt
image::OweActivityDiagram.png[width=500]

==== Implementation of `owe` command
...
The following sequence diagram summarizes what happens during the execution of a `owe` command:

image::OweSequenceDiagram.png[]
.Sequence diagram of the `owe` command
...
As the two commands are similar, it is sufficient to trace the `WalletExpenseCommand`. The following activity diagram illustrates what happens when the user enters an `expense` command:

.Activity diagram of recording an expense
image::ExpenseActivityDiagram.png[width=500]

==== Implementation of `expense` command
...
The following sequence diagram summarizes what happens during the execution of an `expense` command:

.Sequence diagram of the `expense` command
...
* **Alternative 1 (current implementation):** Amounts are stored in dollars as `double`.
...
* **Alternative 2 (future implementation):** Amounts are stored in cents as `long`.
