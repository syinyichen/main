...
[[transactionIndexFAQ]]
*Q*: What is a `transaction's index`? +
*A*: A `transaction's index` is the index number shown in the displayed transaction list in *Sharkie*. It indicates a specific transaction in the address book.

****
For example,

* `1` is the `transaction's index` of Alex Yeoh, whereas
* `2` is the `transaction's index` of Bernice Yu.
